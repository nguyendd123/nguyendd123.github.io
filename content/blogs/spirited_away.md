...
They copy the sequence to ```v1[56]``` using ```sprintf```, and if the ```cnt``` integer has 3 numbers, the consequence will have 57 bytes, thus overwriting ```v2``` to 'n' which is 0x6e. As a result, we can overwrite pointer of the chunk (```v6```) to a fake chunk. Create a fake chunk at ```v7[80]```, and then create a new comment, now you can write 0x6e bytes to ```v7[80]```, which is right above the ebp, overwriting eip :DD.
